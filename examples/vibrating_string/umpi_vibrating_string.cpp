...
// initial velocity
static inline double u_0_dt(double x) { return 0.0; }
...
void f() {
    ...
    N_l = (int *)secure_malloc(C_size * sizeof(*N_l));
    N0_l = (int *)secure_malloc(C_size * sizeof(*N0_l));
    ...
    u_old_l = (double *)secure_malloc((N_l[C_rank]) * sizeof(*u_old_l));
    u_l = (double *)secure_malloc((N_l[C_rank]) * sizeof(*u_l));
    u_new_l = (double *)secure_malloc((N_l[C_rank]) * sizeof(*u_new_l));
    ...
    for(double t = 2 * dt; t <= t_end; t += dt) {
        ...
        MPI_IUsend(&u_new_l[N_l[C_rank] - 2], 1, MPI_DOUBLE, C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL,
            right_copy, MPI_COMM_WORLD, &requests[0]);
        MPI_IUsend(&u_new_l[1], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL, left_copy, MPI_COMM_WORLD,
            &requests[1]);
        MPI_IUrecv(&u_new_l[0], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL, right_copy, MPI_COMM_WORLD,
            &requests[2]);
        MPI_IUrecv(&u_new_l[N_l[C_rank] - 1], 1, MPI_DOUBLE, C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL,
            left_copy, MPI_COMM_WORLD, &requests[3]);
    ...
    if(C_rank == 0) u = (double *)secure_malloc(N * sizeof(*u));
...
int main(int argc, char *argv[]) {
...

    {
        // Warmup
        f();
        if(C_rank == 0) t_start = MPI_Wtime();

        for(size_t i = 0; i < max_iter; i++) { f(); }
