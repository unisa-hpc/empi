...
void f(){
  ...
  for (double t = 2 * dt; t <= t_end; t += dt) {
    ...
    MPI_IUsend(&u_new_l[N_l[C_rank] - 2], 1, MPI_DOUBLE,
              C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL, right_copy, MPI_COMM_WORLD,
              &requests[0]);
    MPI_IUsend(&u_new_l[1], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL,
              left_copy, MPI_COMM_WORLD, &requests[1]);
    MPI_IUrecv(&u_new_l[0], 1, MPI_DOUBLE, C_rank - 1 >= 0 ? C_rank - 1 : MPI_PROC_NULL,
              right_copy, MPI_COMM_WORLD, &requests[2]);
    MPI_IUrecv(&u_new_l[N_l[C_rank] - 1], 1, MPI_DOUBLE,
  ...
  if (C_rank == 0)
    u = (double*) secure_malloc(N * sizeof(*u));
