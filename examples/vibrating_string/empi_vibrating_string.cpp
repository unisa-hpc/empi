...
void f(std::unique_ptr<empi::MessageGroup> &comm_world) {
    ...
    comm_world->run([&](empi::MessageGroupHandler<double, empi::NOTAG, 1> &cgh) {
        ...
        for(double t = 2 * dt; t <= t_end; t += dt) {
            ...
            cgh.Isend(
                &u_new_l[N_l[C_rank] - 2], C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL, empi::Tag{right_copy});
            ...
            cgh.Irecv(
                &u_new_l[N_l[C_rank] - 1], C_rank + 1 < C_size ? C_rank + 1 : MPI_PROC_NULL, empi::Tag{left_copy});
            cgh.waitall();
